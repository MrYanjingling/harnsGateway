...
import (
	...
	"harnsgateway/cmd/consumer/options"
...
func Daemon(manager *data.Manager) (func(ctx context.Context), error) {
	cron := NewWithSeconds()
...
func (s *Server) InstallHandlers() {
	...
	data.InstallHandler(v1, s.Config.DataMgr)
}
...
func (s *Server) Serve() (func(ctx context.Context), error) {
	...
	return func(ctx context.Context) {
		...
		if err := s.Config.DataMgr.Shutdown(ctx); err != nil {
			klog.Error(err)
		}
